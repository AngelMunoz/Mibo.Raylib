...
`DayNightDuration` is the real-time seconds for one full cycle. A 60-second duration means 1 game day = 1 real minute. Adjust for your game's pacing.

### Pre-computed state on the model

The `LightingModel` holds all values the renderer needs:
...
The `DayNight.fs` module uses piecewise linear interpolation between time checkpoints:
...
The same pattern applies to ambient color, light color, and intensity.
...
> _**TIP**_: Keep the interpolation logic in a separate module (`DayNight.fs`). It's pure functions with no dependencies on the game model — easy to test with unit tests.

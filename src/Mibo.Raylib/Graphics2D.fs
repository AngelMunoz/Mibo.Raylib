...
type Batch2DRenderer<'Model>
    ...
    let executeCommands
        ...
        for i = 0 to buffer.Count - 1 do
            ...
            | _, DrawSprite sprite ->
                let spriteCenter =
                    Vector2(sprite.Dest.X + sprite.Dest.Width / 2.0f, sprite.Dest.Y + sprite.Dest.Height / 2.0f)

                let lightColor =
                    Lighting2D.computeLightColor ambient pointLights directionalLights spriteCenter
            ...
            | _, DrawSkyGradient(top, bottom, _) ->
                ...
                Raylib.DrawRectangleGradientV(0, 0, ctx.WindowWidth, ctx.WindowHeight, top, bottom)
    ...
    interface IRenderer<'Model> with
        member _.Draw(ctx, model, gameTime) =
            ...
            | None -> executeCommands ctx ambient (List.ofSeq pointLights) (List.ofSeq directionalLights)
            | Some cfg ->
                let target = getOrCreateTarget ctx.WindowWidth ctx.WindowHeight
                Raylib.BeginTextureMode(target)
                Raylib.ClearBackground(Color.Black)
                executeCommands ctx ambient (List.ofSeq pointLights) (List.ofSeq directionalLights)

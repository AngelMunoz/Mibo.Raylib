...
type RaylibExtensions =
    ...
    [<Extension>]
    static member inline AsBool(c: CBool) : bool =
        CBool.op_Implicit(c)

module RaylibHelpers =
    let inline windowShouldClose() : bool =
        Raylib.WindowShouldClose().AsBool()

    let inline getFrameTime() : float32 =
        Raylib.GetFrameTime()

    let inline isKeyDown (key: KeyboardKey) : bool =
        Raylib.IsKeyDown(key).AsBool()

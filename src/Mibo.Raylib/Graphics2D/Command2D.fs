...
module Command2D =
  ...
  let inline sprite(state: SpriteState) : IRenderCommand2D =
    SpriteCommand(state)
  ...
  [<Struct>]
  type RectRoundedOutlineCommand
    (
      rect: Rectangle,
      roundness: float32,
      segments: int,
  ...
  [<Struct>]
  type FillCircleCommand
    (center: Vector2, radius: float32, color: Color, cmdLayer: int<RenderLayer>)
    =
    interface IRenderCommand2D with
      member _.Layer = cmdLayer
  ...
  [<Struct>]
  type CircleSectorOutlineCommand
    (
      center: Vector2,
      radius: float32,
      startAngle: float32,
  ...
  [<Struct>]
  type LineCommand
    (start: Vector2, finish: Vector2, color: Color, cmdLayer: int<RenderLayer>)
    =
    interface IRenderCommand2D with
      member _.Layer = cmdLayer
  ...
  [<Struct>]
  type LineStripCommand
    (points: Vector2[], color: Color, cmdLayer: int<RenderLayer>) =
    interface IRenderCommand2D with
      member _.Layer = cmdLayer
  ...
  type BezierCommand
    ...
    interface IRenderCommand2D with
      ...
      member _.Render _ =
        Raylib.DrawLineBezier(start, control, thickness, color)
  ...
  type SetScissorCommand
    (x: int, y: int, w: int, h: int, cmdLayer: int<RenderLayer>) =
    interface IRenderCommand2D with
      member _.Layer = cmdLayer

...
module Program =
    ...
    let withConfig
        ...
        =
        { program with Config = configure :: program.Config }

    let withRenderer
        (factory: unit -> IRenderer<'Model>)
        (program: Program<'Model, 'Msg>)
        =
        { program with Renderers = factory :: program.Renderers }

    let withTick map program = {
        program with Tick = ValueSome map
    }
    ...
    let withFixedStep cfg program =
        ...
        { program with FixedStep = ValueSome cfg }

    let withDispatchMode mode program = {

...
module Program =
    ...
    let withConfig (configure: GameConfig -> unit) (program: Program<'Model, 'Msg>) =
        { program with
            Config = configure :: program.Config }

    let withRenderer (factory: unit -> IRenderer<'Model>) (program: Program<'Model, 'Msg>) =
        { program with
    ...
    let withSubscription subscribe program = { program with Subscribe = subscribe }

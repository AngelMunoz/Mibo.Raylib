...
let generateTerrain (seed: int) : Platform list * TorchLight list =
    let rng = Random(seed)
    ...
    while x < 5000.0f do
        ...
        platforms <- { Bounds = Raylib_cs.Rectangle(x, py, segmentLength, tileSize) } :: platforms
    ...
    platforms |> List.rev, torches |> List.rev
...
let init (ctx: GameContext) =
    ...
    let platforms, torches = generateTerrain seed
...
let view (ctx: GameContext) (model: Model) (buffer: RenderBuffer<RenderCmd2D>) =
    ...
    buffer.Add(-1000<RenderLayer>, DrawSkyGradient(topColor, botColor, -1000<RenderLayer>))

...
let createLight (lightType: int) (position: Vector3) (target: Vector3) (color: Color) (attenuation: float32) (shader: Shader) =
    let light = Light()
    if lightsCount >= MAX_LIGHTS then light
    else
        light.Enabled     <- 1
        ...
        let i = lightsCount
        light.EnabledLoc     <- Raylib.GetShaderLocation(shader, sprintf "lights[%d].enabled" i)
        light.TypeLoc        <- Raylib.GetShaderLocation(shader, sprintf "lights[%d].type" i)
        light.PositionLoc    <- Raylib.GetShaderLocation(shader, sprintf "lights[%d].position" i)
        light.TargetLoc      <- Raylib.GetShaderLocation(shader, sprintf "lights[%d].target" i)
        light.ColorLoc       <- Raylib.GetShaderLocation(shader, sprintf "lights[%d].color" i)
        light.AttenuationLoc <- Raylib.GetShaderLocation(shader, sprintf "lights[%d].attenuation" i)
        light.InnerCutoffLoc <- Raylib.GetShaderLocation(shader, sprintf "lights[%d].innerCutoff" i)
        light.OuterCutoffLoc <- Raylib.GetShaderLocation(shader, sprintf "lights[%d].outerCutoff" i)
...
let updateLightValues (shader: Shader) (light: Light) =
    ...
    setShaderVec4    shader light.ColorLoc       (Vector4(float32 light.Color.R / 255.0f,
...
let loadShadowmapRenderTexture (width: int) (height: int) =
    ...
    t.Value <- RenderTexture2D(Id = t.Value.Id, Texture = Texture2D(Width = width, Height = height))
    Rlgl.EnableFramebuffer(t.Value.Id)
    let depthId = Rlgl.LoadTextureDepth(width, height, false)
    t.Value <- RenderTexture2D(
        Id = t.Value.Id,
        Texture = t.Value.Texture,
        Depth = Texture2D(Id = depthId, Width = width, Height = height,
                           Format = enum<PixelFormat> 19, Mipmaps = 1)
    )
    Rlgl.FramebufferAttach(t.Value.Id, depthId, FramebufferAttachType.Depth, FramebufferAttachTextureType.Texture2D, 0)
...
let createShadowCaster (shader: Shader) (resolution: int)
                        (vpName: string) (mapName: string) (resName: string)
                        (slot: int) (projection: CameraProjection) (fovy: float32) =
    let sc = ShadowCaster()
...
let renderShadowPass (sc: ShadowCaster) (shader: Shader)
                     (plane: Model) (cube: Model) (cubePositions: Vector3[]) =
...
let main _ =
    ...
    Raylib.InitWindow(screenWidth, screenHeight, "raylib - shadow mapping (sun + spot)") |> ignore
    ...
    lights.[0] <- createLight LIGHT_DIRECTIONAL Vector3.Zero sunDir Color.White 0.0f shader
    updateLightValues shader lights.[0]

    // 1: spot light shining down
    let spotPos    = Vector3(10.0f, 10.0f, 5.0f)
    let spotDir    = Vector3(0.0f, -1.0f, 0.0f)
    let spotTarget = Raymath.Vector3Add(spotPos, Raymath.Vector3Normalize(spotDir))
    lights.[1] <- createLight LIGHT_SPOT spotPos spotTarget Color.White (1.0f / (30.0f * 30.0f)) shader
    lights.[1].InnerCutoff <- MathF.Cos(30.0f * DEG2RAD)
    lights.[1].OuterCutoff <- MathF.Cos(45.0f * DEG2RAD)
    updateLightValues shader lights.[1]

    // ---- Shadow casters ----
    let sun = createShadowCaster shader SUN_SHADOW_RES
                "lightVP" "shadowMap" "shadowMapResolution"
                10 CameraProjection.Orthographic 30.0f

    let spot = createShadowCaster shader SPOT_SHADOW_RES
                "spotLightVP" "spotShadowMap" "spotShadowMapResolution"
                11 CameraProjection.Perspective
                (MathF.Acos(lights.[1].OuterCutoff) * RAD2DEG * 2.0f)
    spot.Camera <- Camera3D(
    ...
    // ---- Scene ----
    let mutable plane = Raylib.LoadModelFromMesh(Raylib.GenMeshPlane(20.0f, 20.0f, 1, 1))
    let mutable cube  = Raylib.LoadModelFromMesh(Raylib.GenMeshCube(1.0f, 2.0f, 1.0f))
    ...
    while running do
        if Raylib.WindowShouldClose().AsBool() then running <- false
        ...
        sunDir <- Raymath.Vector3Normalize(Vector3(MathF.Cos(t * 0.3f), -1.0f, MathF.Sin(t * 0.3f)))
        lights.[0].Target <- sunDir
        updateLightValues shader lights.[0]

        // Orbit the spot light
        lights.[1].Position <- Vector3(MathF.Cos(t) * 8.0f, 10.0f, MathF.Sin(t) * 8.0f)
        lights.[1].Target   <- Raymath.Vector3Add(lights.[1].Position, Vector3(0.0f, -1.0f, 0.0f))
        updateLightValues shader lights.[1]
        ...
        setShaderVec3 shader (NativePtr.get shader.Locs (int ShaderLocationIndex.VectorView)) viewPos

        // Sun shadow camera follows the player
        sun.Camera <- Camera3D(
            Position = Raymath.Vector3Add(camera.Position, Raymath.Vector3Scale(sunDir, -30.0f)),
        ...
        Raylib.DrawText("Shadow mapping: directional sun + spot light", 10, 10, 18, Color.Black)
